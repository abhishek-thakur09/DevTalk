...
const Chat = () => {
  ...
  // For keeps the track on new messages
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    socket.on("messageReceived", ({ firstName, text }) => {
      ...
      setMessage((messages) => [
        ...messages,
        { firstName: firstName, text, time: new Date().toISOString() },
      ]);
    });
  ...
  const targetConnection = connection?.find((user) => user._id === target_id);
    ...
    <>
      <div className="min-h-screen w-full bg-gradient-to-br from-blue-100 to-blue-300 flex items-center justify-center">
        <div className="w-full max-w-5xl mx-auto border border-white/30  shadow-xl bg-white/20 backdrop-blur-lg h-screen sm:h-[95vh] md:h-[90vh] flex flex-col overflow-hidden">
          {/* Header */}
          <div className="relative border-b border-blue-300 p-3 bg-white/30 backdrop-blur-md">
            <Link
              to="/connections"
            ...
            <h1 className="text-xl sm:text-2xl text-center font-semibold text-gray-800">
          ...
          {/* Chat Body */}
          <div className="flex-1 bg-gradient-to-br from-blue-100 to-blue-300 overflow-y-auto px-4 sm:px-4 py-4 space-y-4 scroll-smooth">
            {(messages || []).map((msg, index) => (
              ...
              >
                <div className="flex flex-col chat-header text-xs font-semibold text-gray-600">
                  {msg.firstName}
          ...
          {/* Chat Input */}
          <div className="p-3 border-t border-gray-200bg-gradient-to-br from-blue-100 to-blue-300 backdrop-blur-sm flex items-center gap-2">
            <input
              value={newmessages}
              onChange={(e) => setnewMessages(e.target.value)}
              className="flex-1 px-4 py-2 rounded-full bg-gray-200 focus:outline-none focus:ring-2 focus:ring-blue-400 text-sm"
